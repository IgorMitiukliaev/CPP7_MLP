...
void MainWindow::on_CreateGraph_clicked() {
    _graphWindow->show();
    std::vector<double> v = {0.4f, 0.6f, 0.7f};
...
auto MainWindow::ResearchInitGraphPerceptron() -> void {
    ...
    _controller->InitNetwork(config);
    _controller->LoadConfiguration(QDir::currentPath().append("/../src/conf.bin").toStdString(), true);
    _controller->loadDataset(QDir::currentPath().append("/../datasets/emnist-letters/emnist-letters-test.csv").toStdString());
}
...
auto MainWindow::ResearchInitMatrixPerceptron() -> void {
    ...
    config.num_neurons_out = 26;
    _controller->InitNetwork(config);
    _controller->LoadConfiguration(QDir::currentPath().append("/../src/conf.bin").toStdString(), false);
    _controller->loadDataset(QDir::currentPath().append("/../datasets/emnist-letters/emnist-letters-test.csv").toStdString());
}
...
void MainWindow::on_pushButtonResearch_clicked() {
    for (int i = 1; i <= 2; ++i) {
        ...
        for (int j = 1; j <= 3; ++j) {
            ...
            averageTime += time;
            ui->tableWidget->setItem(i, j, new QTableWidgetItem(QString(QString::number(time).append(" sec"))));
        }
        averageTime /= 1110;
        ui->tableWidget->setItem(i, 4, new QTableWidgetItem(QString(QString::number(averageTime).append(" sec"))));
    }
}
