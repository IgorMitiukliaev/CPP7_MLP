...
double GraphNeuralNetwork::Neuron::sigmoid(double x) { return 1 / (1 + exp(-x)); }
double GraphNeuralNetwork::Neuron::dsigmoid(double x) { return sigmoid(x) * (1 - sigmoid(x)); }
...
GraphNeuralNetwork::Neuron::Neuron(std::vector<Neuron> *input_layer) : Neuron() {
...
void GraphNeuralNetwork::Neuron::evaluateErr(unsigned int num_pos = 0, double correct = 0) {
    delta_ = 0;
    if (p[0] == nullptr) {
...
void GraphNeuralNetwork::Neuron::refreshWeight(double const &a_, double const &g_) {
